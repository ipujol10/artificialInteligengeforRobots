...
class plan:
    ...
    def make_heuristic(self, grid, goal, cost):
        self.heuristic = [[0 for _ in range(len(grid[0]))]
    ...
    def astar(self):
        ...
        closed = [[0 for _ in range(len(self.grid[0]))]
                  for _ in range(len(self.grid))]
        action = [[0 for _ in range(len(self.grid[0]))]
                  for _ in range(len(self.grid))]
    ...
    def smooth(self, weight_data=0.1, weight_smooth=0.1,
        ...
        self.spath = [[0 for _ in range(len(self.path[0]))]
...
class particles:
    ...
    def sense(self, Z):
        w = [self.data[i].measurement_prob(Z) for i in range(self.N)]
...
def getVectors(index, spath, estimate):
    p1 = spath[index]
    p2 = spath[index + 1]
    delta = [p2[0] - p1[0], p2[1] - p1[1]]
    r = [estimate[0] - p1[0], estimate[1] - p1[1]]
    return delta, r
...
def run(grid, goal, spath, params, printflag=False, speed=0.1, timeout=1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        u = (r[0]*delta[0] + r[1]*delta[1]) / \
            sqrt(delta[0] ** 2 + delta[1] ** 2)
        if (u > 1):
            index += 1
        delta, r = getVectors(index, spath, estimate)
        cte = (r[1]*delta[0] - r[0]*delta[1]) / \
            sqrt(delta[0] ** 2 + delta[1] ** 2)
